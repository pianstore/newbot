...
${global.group.variable}`, 
        ...
        if (isQuoted) {
            if (isQuoted.type === 'text') {
                text = isQuoted.text || '';
                keyword = content.trim();
            } else if (isQuoted.type === 'image') {
                text = isQuoted.content?.caption || '';
                keyword = content.trim();
            }
        } else {
            // Memastikan tidak terjadi re-deklarasi variabel keyword dan text
            [keyword, text] = content.split('|').map(item => item.trim());
        }
