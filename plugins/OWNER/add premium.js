...
async function handle(sock, messageInfo) {
    const { remoteJid, message, sender, mentionedJid, isQuoted, content, prefix, command } = messageInfo;


    try {
        // Validasi input
        if (!content || content.trim() === '') {
            const tex = `_⚠️ Format Penggunaan:_ \n\n_💬 Contoh:_ _*${prefix + command} 6285246154386 30*_`;
        ...
        if (!/^\d{10,15}$/.test(nomorHp)) {
            return await sock.sendMessage(remoteJid, { text: `_Nomor tidak valid. Pastikan formatnya benar_\n\n_Contoh: *${prefix + command} 628xxx* 30_` }, { quoted: message });
        }
        ...
        const addedPremiumTime = currentDate.setDate(currentDate.getDate() + parseInt(jumlahHariPremium)); // Menambahkan hari
        ...
        const responseText = `_Masa Premium pengguna_ @${nomorHp.split('@')[0]} _telah diperpanjang hingga:_ ${premiumEndDate.toLocaleString()}`;
